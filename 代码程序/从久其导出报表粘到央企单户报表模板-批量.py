...
filearray=[]
filelocation=glob.glob(r"E:\久其导表\单户表\*XLS") #久其导出报表所在文件夹
for filename in filelocation:
    wb1 = app.books.open(filename)
    bookname = filename.split("\\")[-1][:-4]
    print(bookname)
    sht1 = wb1.sheets["FMDM 封面代码"]
    wb2 = app.books.open(r'E:\报表模板\单户报表模板.xlsx')  # TB模板位置
    ...
    s = r"生成报表"
    wb2.save((r"E:\生成报表\2-{}2021年度财务报表.xlsx" ).format(bookname))
    wb2.close()
    wb1.close()
