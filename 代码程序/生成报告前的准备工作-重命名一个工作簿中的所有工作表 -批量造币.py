...
filearray=[]
filelocation=glob.glob(r"D:\2021年度审计\TB模板\21年久其TB\*XLS") #久其导出报表所在文件夹
for filename in filelocation:
        ...
        if name == "QCF10 受限制的货币资金明细":
            ...
            while n0<24:
                    if  i.range(n0, 1).value=="合    计":
                        ...
                        qt = i.range(n0, m0).value-i.range(4, m0).value-i.range(5, m0).value-i.range(6, m0).value-i.range(7, m0).value-i.range(8, m0).value-i.range(9, m0).value-i.range(10, m0).value
    ...
    workbook.save(r"D:\2021年度审计\TB模板\21年替换空格后久其TB\{}".format(bookname))
    workbook.close()
